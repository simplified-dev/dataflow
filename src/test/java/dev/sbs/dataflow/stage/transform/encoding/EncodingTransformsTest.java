...
class EncodingTransformsTest {
    ...
    void base64RoundTrip() {
        String encoded = Base64EncodeTransform.create().execute(this.ctx, "hello world");
        assertThat(encoded, is(equalTo("aGVsbG8gd29ybGQ=")));
        assertThat(Base64DecodeTransform.create().execute(this.ctx, encoded), is(equalTo("hello world")));
    }

    @Test
    @DisplayName("Base64 decode of garbage returns null")
    void base64DecodeBadInput() {
        assertThat(Base64DecodeTransform.create().execute(this.ctx, "###"), is(nullValue()));
    }

    @Test
    @DisplayName("URL encode/decode round-trips reserved characters")
    void urlRoundTrip() {
        String encoded = UrlEncodeTransform.create().execute(this.ctx, "a b/c?d");
        assertThat(encoded, is(equalTo("a+b%2Fc%3Fd")));
        assertThat(UrlDecodeTransform.create().execute(this.ctx, encoded), is(equalTo("a b/c?d")));
    }

    @Test
    @DisplayName("URL decode of malformed escape returns null")
    void urlDecodeBadInput() {
        assertThat(UrlDecodeTransform.create().execute(this.ctx, "%ZZ"), is(nullValue()));
    }
